...
let http_client_roundtrip () =
  ...
  let client_src =
    "fn main() {\n\
    \    with [ HttpClient <- BlockingHttpClient ] @ 'Process {\n\
...
(* The graceful-shutdown demo: a `Router` STRUCT whose `.get`/`.post`/`.dispatch`
   ...
   `with`-block shutdown defer that fires after the accept loop returns. *)
let router_graceful () =
  let port = 18080 in
  ...
  (* Deterministic teardown: per-request defer + with-block shutdown defer. *)
