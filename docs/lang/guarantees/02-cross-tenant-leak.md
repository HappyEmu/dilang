...
A junior writes the worker by lifting code out of the request handler. They drop the request-scope wrappers, since "this isn't a request":
...
`TenantBilling` is declared `@ Tenant` (design §2.8.2). The worker's call to `send_invoice` happens inside a `@ Job` scope with no enclosing `@ Tenant`. The compiler walks the requires row outward looking for a binding and finds none:
...
The only way to satisfy both errors is to nest a `@ Tenant` scope inside the `@ Job` scope, constructed from data carried by the job:
...
The compiler enforces that a `@ Tenant` scope exists around any `TenantBilling` call, and that the scope's Lifecycle hooks fire on entry and exit. It does **not** enforce that the `tenant_id` passed to `PgBilling` is the *right* one for the job — that is runtime data, and the compiler treats `job.tenant_id` as opaque.
...
- [Vignette 01](./01-job-vs-request-scope.md) — the same scope mechanism applied to request-scoped state
